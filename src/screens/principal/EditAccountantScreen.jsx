  ...
  return (
    <ScrollView
      ...
      <Animated.View entering={FadeInDown.duration(260).springify()} style={styles.hero}>
        ...
        <Text style={styles.heroTitle} numberOfLines={1}>{form.fullName || 'Edit Accountant'}</Text>
        <Text style={styles.heroSub}>{form.employeeId ? `Employee ID: ${form.employeeId}` : 'Update accountant profile'}</Text>
      </Animated.View>
      ...
      <Animated.View entering={FadeInDown.delay(80).duration(260).springify()} style={styles.formCard}>
        ...
        <View style={styles.selectWrap}>
          <SelectField
            ...
            onChange={v => updateField('isActive', v === 'ACTIVE')}
          />
        </View>
      </Animated.View>
      ...
      <Pressable
        ...
        <Text style={styles.submitBtnText}>{mutation.isPending ? 'Saving…' : 'Save Accountant'}</Text>
      </Pressable>
      ...
      <ConfirmationModal
        ...
        title="Save Changes?"
        message={`Update accountant profile for ${form?.fullName || 'this accountant'}?`}
